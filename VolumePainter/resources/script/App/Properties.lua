...
function Properties:Update()
    ...
    if not Viewport.selection_set or #Viewport.selection_set == 0 or not Viewport.volumeWorld then
    ...
    for i, ent in ipairs(Viewport.selection_set) do
        hasComp[COMPONENTS.TRANSFORM] = hasComp[COMPONENTS.TRANSFORM] and Viewport.volumeWorld.coreWorld.transform:HasComponent(ent)
        hasComp[COMPONENTS.LIGHT] = hasComp[COMPONENTS.LIGHT] and Viewport.volumeWorld.coreWorld.light:HasComponent(ent)
        hasComp[COMPONENTS.GLIGHT] = hasComp[COMPONENTS.GLIGHT] and Viewport.volumeWorld.coreWorld.globalLight:HasComponent(ent)
        hasComp[COMPONENTS.STATIC] = hasComp[COMPONENTS.STATIC] and Viewport.volumeWorld.coreWorld.staticMesh:HasComponent(ent)
        hasComp[COMPONENTS.COLLISION] = hasComp[COMPONENTS.COLLISION] and Viewport.volumeWorld.coreWorld.collision:HasComponent(ent)
        hasComp[COMPONENTS.ENVPROB] = hasComp[COMPONENTS.ENVPROB] and Viewport.volumeWorld.coreWorld.envprobs:HasComponent(ent)
        
        if hasComp[COMPONENTS.SCRIPT] then
            luaEntity = Viewport.volumeWorld.coreWorld.script:GetLuaEntity(ent)
            if luaEntity == nil then 
                has_script = false
            else
                local currentType = Viewport.volumeWorld.coreWorld:GetEntityType(ent)
                if entType == nil then 
                    entType = currentType
                else
	...
	for i, has in ipairs(hasComp) do
		if has then
			if i ~= COMPONENTS.SCRIPT then
				...
				if entType ~= nil then
					local varsCount = Viewport.volumeWorld.coreWorld.script:GetLuaVarsCount(luaEntity.ent)
					local varsNames = {}
					for i = 0, varsCount - 1 do
						varsNames[#varsNames + 1] = Viewport.volumeWorld.coreWorld.script:GetLuaVarName(luaEntity.ent, i)
					end
...
function Properties:AddMenuClick(btn, ev)
    ...
    if ev.id == "ac_transform" then
		for i, ent in ipairs(Viewport.selection_set) do
            Viewport.volumeWorld.coreWorld.transform:AddComponent(ent)
		end

    elseif ev.id == "ac_vis" then
		for i, ent in ipairs(Viewport.selection_set) do
            Viewport.volumeWorld.coreWorld.visibility:AddComponent(ent)
		end
		
    elseif ev.id == "ac_earlyvis" then
		for i, ent in ipairs(Viewport.selection_set) do
            Viewport.volumeWorld.coreWorld.earlyVisibility:AddComponent(ent)
		end
		
    elseif ev.id == "ac_light" then
		for i, ent in ipairs(Viewport.selection_set) do
            Viewport.volumeWorld.coreWorld.light:AddComponent(ent)
		end
		
    elseif ev.id == "ac_glight" then
		for i, ent in ipairs(Viewport.selection_set) do
            Viewport.volumeWorld.coreWorld.globalLight:AddComponent(ent)
		end
		
    elseif ev.id == "ac_mesh" then
		for i, ent in ipairs(Viewport.selection_set) do
            Viewport.volumeWorld.coreWorld.staticMesh:AddComponent(ent)
		end
				
    elseif ev.id == "ac_script" then
		for i, ent in ipairs(Viewport.selection_set) do
            Viewport.volumeWorld.coreWorld.script:AddComponent(ent)
		end
		
    elseif ev.id == "ac_collision" then
		for i, ent in ipairs(Viewport.selection_set) do
            Viewport.volumeWorld.coreWorld.collision:AddComponent(ent)
		end
		
    elseif ev.id == "ac_envprob" then
		for i, ent in ipairs(Viewport.selection_set) do
            Viewport.volumeWorld.coreWorld.envprobs:AddComponent(ent)
		end
