...
bool SceneRenderMgr::RegMesh(uint32_t indexCount, RHI::GfxBuffer* indexBuffer, RHI::GfxBuffer* vertexBuffer,
...
bool SceneRenderMgr::RegPointCaster(uint8_t type, Vector4& color, Vector4& nonAreaColor, float range, Vector3& area, Vector3& pos, Vector3& dir, Vector4& farNear, CXMMATRIX proj, CXMMATRIX view, uint64_t id)
{
	...
	auto viewMatrix = XMMatrixSet(1.0f, 0, 0, -pos.x, 0, 1.0f, 0, -pos.y, 0, 0, 1.0f, -pos.z, 0, 0, 0, 1.0f);
	...
	casterPoint_array[casterPoint_count].matView = viewMatrix;
